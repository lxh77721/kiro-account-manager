...
const isContainerDesktop = process.env.KIRO_CONTAINER_DESKTOP === 'true'
const isTrayDisabled = isContainerDesktop || process.env.KIRO_DISABLE_TRAY === 'true'
const isAutoUpdateDisabled = isContainerDesktop || process.env.KIRO_DISABLE_AUTO_UPDATE === 'true'
const areGlobalShortcutsDisabled =
  isContainerDesktop || process.env.KIRO_DISABLE_GLOBAL_SHORTCUTS === 'true'
...
function registerShowWindowShortcut(): void {
  ...
  if (areGlobalShortcutsDisabled) {
    console.log('[Shortcut] Global shortcuts disabled by environment')
    return
  }
...
function createWindow(): void {
  ...
  const windowWidth = Math.max(800, Math.min(defaultWidth, workAreaSize.width - 48))
  const windowHeight = Math.max(600, Math.min(defaultHeight, workAreaSize.height - 48))
...
app.whenReady().then(async () => {
  ...
  // 初始化自动更新（仅生产环境）
  if (!is.dev && !isAutoUpdateDisabled) {
    setupAutoUpdater()
    // 启动后延迟检查更新
    setTimeout(() => {
  ...
  ipcMain.handle('check-for-updates', async () => {
    if (is.dev || isAutoUpdateDisabled) {
      return { hasUpdate: false, message: '当前运行模式不支持更新检查' }
    }
    try {
  ...
  ipcMain.handle('download-update', async () => {
    if (is.dev || isAutoUpdateDisabled) {
      return { success: false, message: '当前运行模式不支持更新' }
    }
    try {
