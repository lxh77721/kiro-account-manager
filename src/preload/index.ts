...
const api = {
  ...
  // 账号管理 - 保存账号数据
  saveAccounts: (data: unknown, options?: { allowEmptyAccounts?: boolean }): Promise<void> => {
    return ipcRenderer.invoke('save-accounts', data, options)
  },
  ...
  // 同步账号到反代池（批量更新）
  proxySyncAccounts: (accounts: Array<{
    id: string
    email?: string
    nickname?: string
    userId?: string
  ...
  // 获取账户可用模型列表
  accountGetModels: (accessToken: string, region?: string, profileArn?: string, accountId?: string): Promise<{ success: boolean; error?: string; models: Array<{ id: string; name: string; description: string; inputTypes?: string[]; maxInputTokens?: number | null; maxOutputTokens?: number | null; rateMultiplier?: number; rateUnit?: string }> }> => {
    return ipcRenderer.invoke('account-get-models', accessToken, region, profileArn, accountId)
  },

  // 获取可用订阅列表
  accountGetSubscriptions: (accessToken: string, region?: string, accountId?: string): Promise<{ success: boolean; error?: string; plans: Array<{ name: string; qSubscriptionType: string; description: { title: string; billingInterval: string; featureHeader: string; features: string[] }; pricing: { amount: number; currency: string } }>; disclaimer?: string[] }> => {
    return ipcRenderer.invoke('account-get-subscriptions', accessToken, region, accountId)
  },

  // 获取订阅管理/支付链接
  accountGetSubscriptionUrl: (accessToken: string, subscriptionType?: string, region?: string, accountId?: string): Promise<{ success: boolean; error?: string; url?: string; status?: string }> => {
    return ipcRenderer.invoke('account-get-subscription-url', accessToken, subscriptionType, region, accountId)
  },
