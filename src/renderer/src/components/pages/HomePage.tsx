...
export function HomePage() {
  ...
  const darkMode = useAccountsStore((state) => state.darkMode)
  const usagePrecision = useAccountsStore((state) => state.usagePrecision)
  ...
  const stats = useMemo(() => {
    ...
    for (const account of accounts.values()) {
      ...
      if (
        ...
        account.subscription.daysRemaining >= 0
      ) {
        nextStats.expiringSoonCount++
      }
  ...
  const activeAccount = useMemo(
    () => Array.from(accounts.values()).find((account) => account.isActive) as Account | undefined,
    [accounts]
  )
