...
export function AddAccountDialog({ isOpen, onClose }: AddAccountDialogProps): React.ReactNode {
  ...
  // 导入模式
  const [importMode, setImportMode] = useState<ImportMode>(webMode ? 'oidc' : 'login')
  ...
  const resetForm = () => {
    setImportMode(webMode ? 'oidc' : 'login')
    setRefreshToken('')
    setClientId('')
    setClientSecret('')
    setRegion('us-east-1')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <Card className="relative w-full max-w-lg max-h-[90vh] overflow-auto z-10">
        <CardHeader className="pb-4 border-b">
          ...
          {webMode && (
            <div className="p-3 rounded-xl border border-blue-200/70 bg-blue-50/70 text-sm text-blue-700 dark:border-blue-900/40 dark:bg-blue-950/30 dark:text-blue-300">
              {isEn
                ? 'Web mode keeps manual import paths. Browser-assisted login has been disabled.'
                : 'Web 版保留手动导入与批量导入，桌面端浏览器授权登录已禁用。'}
            </div>
          )}
        ...
        <CardContent className="space-y-6 pt-6">
          {/* 导入模式切换 */}
          <div className={`grid gap-1 p-1 bg-muted/50 rounded-xl border ${webMode ? 'grid-cols-2' : 'grid-cols-3'}`}>
            {!webMode && <button
