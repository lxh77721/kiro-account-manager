...
interface AccountsActions {
  ...
  saveToStorage: (options?: { allowEmptyAccounts?: boolean }) => Promise<void>
...
export const useAccountsStore = create<AccountsStore>()((set, get) => ({
  ...
  updateAccount: (id, updates) => {
    ...
    get().saveToStorage({ allowEmptyAccounts: get().accounts.size === 0 })
  },
  ...
  batchRefreshTokens: async (ids) => {
    const { accounts, autoRefreshConcurrency, autoRefreshSyncInfo } = get()
  ...
  loadFromStorage: async () => {
    set({ isLoading: true, storageReady: false })
  ...
  saveToStorage: async (options) => {
    ...
    if (!storageReady && accounts.size === 0) {
      console.warn('[Store] Skip saving empty state before initial storage load finishes')
      return
    }

    if (!allowEmptyAccounts && accounts.size === 0) {
      console.warn('[Store] Refusing to save empty account set without explicit confirmation')
      return
    }
  ...
  handleBackgroundRefreshResult: (data) => {
    ...
    set((state) => {
      ...
      const nextAccessToken = refreshData?.accessToken || refreshData?.newCredentials?.accessToken
      const nextRefreshToken = refreshData?.refreshToken || refreshData?.newCredentials?.refreshToken
      const nextExpiresAt = refreshData?.newCredentials?.expiresAt
        || (refreshData?.expiresIn ? now + refreshData.expiresIn * 1000 : undefined)

      accounts.set(id, {
        ...account,
        credentials: {
          ...account.credentials,
          accessToken: nextAccessToken || account.credentials.accessToken,
          refreshToken: nextRefreshToken || account.credentials.refreshToken,
          expiresAt: nextExpiresAt || account.credentials.expiresAt
        },
